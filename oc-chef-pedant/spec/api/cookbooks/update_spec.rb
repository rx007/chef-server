      ...
      context "as admin user" do
        it "should respond with 200 Ok", :smoke do
          ...
          put(api_url("/#{cookbook_url_base}/#{cookbook_name}/#{cookbook_version}"),
              admin_user, api_version: api_version, :payload => payload) do |response|
            response.
              should look_like({
              :status => 200,
              :body_exact => payload
        ...
        context 'as a user outside of the organization', :authorization do
          ...
          it "should respond with 403 (\"Forbidden\") and does not update cookbook" do
            put(request_url, outside_user, api_version: api_version, :payload => updated_cookbook) do |response|
        ...
        context 'with invalid user', :authentication do
          ...
          it "returns 401 and does not update cookbook" do
            put(api_url("/#{cookbook_url_base}/#{cookbook_name}/#{cookbook_version}"), invalid_user, api_version: api_version, :payload => updated_cookbook) do |response|
              response.should look_like expected_response
            end

            # Verified change did not happen
            get(api_url("/#{cookbook_url_base}/#{cookbook_name}/#{cookbook_version}"), admin_user, api_version: api_version) do |response|
              response.
                should look_like({
                :status => 200,
      ...
      context "for checksums" do
                                                        ...
                                                        {"name" => "name2", "path" => "files/default/name2",
                                                         ...
                                                         {"name" => "name3", "path" => "files/default/name3",
                                                          ...
                                                          {"name" => "name4", "path" => "files/default/name4",
                                                           "checksum" => checksums[3],
                                                           "specificity" => "default"}]})
          upload_cookbook(admin_user, cookbook_name, cookbook_version, payload, api_version: api_version)
        ...
        context "CHEF-3716 coverage" do
          ...
          it "it does not delete checksums in use by another version" do
            ...
            payload1 = new_cookbook(cookbook_name, cookbook_version, api_version: api_version,
                                    payload: {"files" => [{"name" => "name1", "path" => "path/name1",
                                                           "checksum" => checksums[0],
                                                           "specificity" => "default"},
                                                           {"name" => "name2", "path" => "path/name2",
            ...
            payload2 = new_cookbook(cookbook_name, cookbook_version2, api_version: api_version,
                                    payload: { "files" => [{"name" => "name1", "path" => "path/name1",
                                                            "checksum" => checksums[0],
                                                            "specificity" => "default"},
                                                            {"name" => "name2", "path" => "path/name2",
            ...
            upload_cookbook(admin_user, cookbook_name, cookbook_version, payload1, api_version: api_version)
            upload_cookbook(admin_user, cookbook_name, cookbook_version2, payload2, api_version: api_version)

            # compute an intersection and difference
            cbv_1_checksums = checksums_for_segment_type(:files, cookbook_version, api_version: api_version)
            cbv_2_checksums = checksums_for_segment_type(:files, cookbook_version2, api_version: api_version)
      ...
      context "when modifying data" do
        context "for cookbook_name" do
          [1, true, [], {}].each do |value|
            should_fail_to_change('cookbook_name', value, 400, "Field 'cookbook_name' invalid", api_version: api_version)
          end
          ['new_cookbook_name', 'with a space', '外国語'].each do |value|
            should_fail_to_change('cookbook_name', value, 400, "Field 'cookbook_name' invalid", api_version: api_version)
          end
          should_fail_to_change('cookbook_name', :delete, 400, "Field 'cookbook_name' missing", api_version: api_version)
        end # context for cookbook_name

        context "for json_class" do
          should_not_change('json_class', :delete, 'Chef::CookbookVersion', api_version: api_version)
          should_fail_to_change('json_class', 1, 400, "Field 'json_class' invalid", api_version: api_version)
          should_fail_to_change('json_class', 'Chef::NonCookbook', 400, "Field 'json_class' invalid", api_version: api_version)
          should_fail_to_change('json_class', 'all wrong', 400, "Field 'json_class' invalid", api_version: api_version)
        end # context for json_class

        context "for chef_type" do
          should_not_change('chef_type', :delete, 'cookbook_version', api_version: api_version)
          should_fail_to_change('chef_type', 'not_cookbook', 400, "Field 'chef_type' invalid", api_version: api_version)
          should_fail_to_change('chef_type', false, 400, "Field 'chef_type' invalid", api_version: api_version)
          should_fail_to_change('chef_type', ['just any', 'old junk'], 400, "Field 'chef_type' invalid", api_version: api_version)
        end # context for chef_type

        context "for version" do
          should_change('version', :delete, api_version: api_version)
          error = "Field 'version' invalid"
          should_fail_to_change('version', 1, 400, error, api_version: api_version)
          should_fail_to_change('version', ['all', 'ignored'], 400, error, api_version: api_version)
          should_fail_to_change('version', {}, 400, error, api_version: api_version)

          error = "Field 'version' invalid"
          should_fail_to_change('version', '0.0', 400, error, api_version: api_version)
          should_fail_to_change('version', 'something invalid', 400, error, api_version: api_version)
          ...
          ['attributes', 'definitions', 'files', 'libraries', 'providers', 'recipes',
           'resources', 'root_files', 'templates'].each do |segment|
            context "for #{segment}" do
              should_fail_to_change(segment, 'foo', 400, "Field '#{segment}' invalid", api_version: api_version)
              error = "Invalid element in array value of '#{segment}'."
              should_fail_to_change(segment, ['foo'], 400, error, api_version: api_version)
              should_change(segment, [], api_version: api_version)
              should_fail_to_change(segment, [{}, {}], 400, error, api_version: api_version)
              should_fail_to_change(segment, [{'foo' => 'bar'}], 400, error, api_version: api_version)
        ...
        context "for other stuff" do
          should_change('frozen?', true, api_version: api_version)
        end # context for other stuff
      end # context when modifying data

      context "when modifying metadata" do
        should_fail_to_change('metadata', {'new_name' => 'foo'}, 400, "Field 'metadata.version' missing", api_version: api_version)

        context "for name" do
          should_change_metadata('name', 'new_name', nil, 200, :validation, api_version: api_version)
          should_change_metadata('name', :delete, api_version: api_version)
          [[1, 'number'], [true, 'boolean'], [{}, 'object'],
           [[], 'array']].each do |error|
            json_error = "Field 'metadata.name' invalid"
            should_fail_to_change_metadata('name', error[0], 400, json_error, api_version: api_version)
          end
          ['invalid name', 'ダメよ'].each do |name|
            should_fail_to_change_metadata('name', name, 400, "Field 'metadata.name' invalid", api_version: api_version)
          end
        end # context for name

        context "for description" do
          should_change_metadata('description', 'new description', api_version: api_version)
          should_change_metadata('description', :delete, api_version: api_version)
          should_fail_to_change_metadata('description', 1, 400, "Field 'metadata.description' invalid", api_version: api_version)
        end # context for description

        context "for long description" do
          should_change_metadata('long_description', 'longer description', api_version: api_version)

          # Deleting the long description results in it being "reset" to
          # the empty string
          should_change_metadata('long_description', :delete, "", api_version: api_version)
          should_fail_to_change_metadata('long_description', false, 400, "Field 'metadata.long_description' invalid", api_version: api_version)
        end # context for long description

        context "for version" do
          should_fail_to_change_metadata('version', '0.0', 400, "Field 'metadata.version' invalid", api_version: api_version)
          should_fail_to_change_metadata('version', 'not a version', 400, "Field 'metadata.version' invalid", api_version: api_version)
          should_fail_to_change_metadata('version', :delete, 400, "Field 'metadata.version' missing", api_version: api_version)
          should_fail_to_change_metadata('version', 1, 400, "Field 'metadata.version' invalid", api_version: api_version)
        end # context for version

        context "for maintainer" do
          should_change_metadata('maintainer', 'Captain Stupendous', api_version: api_version)
          should_change_metadata('maintainer', :delete, api_version: api_version)
          should_fail_to_change_metadata('maintainer', true, 400, "Field 'metadata.maintainer' invalid", api_version: api_version)
          should_change_metadata('maintainer_email', 'cap@awesome.com', api_version: api_version)
          should_change_metadata('maintainer_email', 'not really an email', api_version: api_version)
          should_change_metadata('maintainer_email', :delete, api_version: api_version)
          should_fail_to_change_metadata('maintainer_email', false, 400, "Field 'metadata.maintainer_email' invalid", api_version: api_version)
        end # context for maintainer

        context "for license" do
          should_change_metadata('license', 'to_kill', api_version: api_version)
          should_change_metadata('license', :delete, api_version: api_version)
          should_fail_to_change_metadata('license', 1, 400, "Field 'metadata.license' invalid", api_version: api_version)
        end # context for license

        context "for collections" do
          context "for platforms" do
            json_error = "Field 'metadata.platforms' invalid"
            should_fail_to_change_metadata('platforms', [], 400, json_error, api_version: api_version)
            should_change_metadata('platforms', {}, api_version: api_version)
            should_change_metadata('platforms', :delete, api_version: api_version)
            should_fail_to_change_metadata('platforms', "foo", 400, json_error, api_version: api_version)
            should_fail_to_change_metadata('platforms', ["foo"], 400, json_error, api_version: api_version)
            should_fail_to_change_metadata('platforms', {"foo" => {}}, 400, "Invalid value '{[]}' for metadata.platforms", api_version: api_version)
          end

          def self.should_change_with_metadata(_attribute, _value, api_version: "2")
            context "when #{_attribute} is set to #{_value}" do
              ...
              should_change_metadata _attribute, _value, _value, 200, api_version: api_version
            end
          end
          ...
          context "with metadata.providing" do
            ...
            should_change_with_metadata 'providing', 'cats::sleep', api_version: api_version
            should_change_with_metadata 'providing', 'here(:kitty, :time_to_eat)', api_version: api_version
            should_change_with_metadata 'providing', 'service[snuggle]', api_version: api_version
            should_change_with_metadata 'providing', '', api_version: api_version
            should_change_with_metadata 'providing', 1, api_version: api_version
            should_change_with_metadata 'providing', true, api_version: api_version
            should_change_with_metadata 'providing', ['cats', 'sleep', 'here'], api_version: api_version
            should_change_with_metadata 'providing',
              { 'cats::sleep'                => '0.0.1',
                'here(:kitty, :time_to_eat)' => '0.0.1',
                'service[snuggle]'           => '0.0.1'  }, api_version: api_version

          end

          context "for dependencies" do
            json_error = "Field 'metadata.dependencies' invalid"
            should_fail_to_change_metadata("dependencies", [], 400, json_error, api_version: api_version)
            should_change_metadata("dependencies", {}, api_version: api_version)

            should_change_metadata("dependencies", :delete, {}, api_version: api_version)

            should_fail_to_change_metadata("dependencies", "foo", 400, json_error, api_version: api_version)
            should_fail_to_change_metadata("dependencies", ["foo"], 400, json_error, api_version: api_version)
            should_fail_to_change_metadata("dependencies", {"foo" => {}}, 400, "Invalid value '{[]}' for metadata.dependencies", api_version: api_version)
